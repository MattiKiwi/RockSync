...
def parse_args(argv: List[str]) -> argparse.Namespace:
    ...
    p.add_argument('--manual-chapters', metavar='PATH', help='Manual chapters metadata JSON (generated by GUI)')
...
def _load_manual_entries(path: str) -> Dict[str, Dict[str, Any]]:
    try:
    ...
    for item in videos:
        ...
        info = item.get('info')
        if not vid or not isinstance(info, dict):
            continue
...
def _normalize_manual_chapters(info: Dict[str, Any]) -> Dict[str, Any]:
    data = dict(info)
    ...
    normalized.sort(key=lambda c: c['start_time'])
    for idx, ch in enumerate(normalized):
        if idx + 1 < len(normalized) and (ch.get('end_time') is None or ch['end_time'] <= ch['start_time']):
            ch['end_time'] = normalized[idx + 1]['start_time']
        if idx + 1 >= len(normalized) and (ch.get('end_time') is None or ch['end_time'] <= ch['start_time']):
            if duration and duration > ch['start_time']:
                ch['end_time'] = duration
            else:
                ch['end_time'] = ch['start_time'] + 1
...
def _manual_entry_url(meta: Dict[str, Any], vid: str) -> str:
    url = str(meta.get('url') or '').strip()
    info = meta.get('info') if isinstance(meta.get('info'), dict) else {}
    if not url and isinstance(info, dict):
...
def _run_manual_chapter_jobs(base_cmd: List[str], manual_entries: Dict[str, Dict[str, Any]], target_root: Path) -> int:
    ...
    def _execute(tokens: List[str]) -> tuple[int, List[str]]:
        captured: List[str] = []
        try:
            proc = subprocess.Popen(tokens, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True)
            print(tokens)
            ...
            msg = f"[yt_download] Manual chapters: error launching yt-dlp: {exc}"
            print(msg)
            return 1, captured + [msg]

    def _strip_format(tokens: List[str]) -> List[str]:
        out: List[str] = []
        skip_next = False
        for idx, tok in enumerate(tokens):
            if skip_next:
                skip_next = False
                continue
            if tok in ('-f', '--format') and idx + 1 < len(tokens):
                skip_next = True
                continue
            if tok.startswith('-f=') or tok.startswith('--format='):
                continue
            out.append(tok)
        return out

    for vid, meta in manual_entries.items():
        info = meta.get('info')
        if not isinstance(info, dict):
            continue
        info = _normalize_manual_chapters(info)
        tmp_dir = None
        if target_root.exists() and target_root.is_dir():
            tmp_dir = str(target_root)
        try:
            tmp = tempfile.NamedTemporaryFile('w', encoding='utf-8', delete=False,
                                              suffix=f'.{vid}.info.json', dir=tmp_dir)
            json.dump(info, tmp, ensure_ascii=False, default=str)
            tmp_path = tmp.name
            tmp.close()
        except Exception as exc:
            print(f"[yt_download] Manual chapters: could not create temp info for {vid}: {exc}")
            overall_rc = overall_rc or 1
            continue
        manual_cmd = list(base_cmd)
        if not _has_flag(base_cmd, '--split-chapters'):
            manual_cmd.append('--split-chapters')
        if not _has_flag(base_cmd, '--force-overwrites'):
            manual_cmd.append('--force-overwrites')
        if not _has_flag(base_cmd, '--write-info-json'):
            manual_cmd.append('--write-info-json')
        if not _has_flag(base_cmd, '--write-thumbnail'):
            manual_cmd.append('--write-thumbnail')
        if not _has_flag(base_cmd, '--convert-thumbnails'):
            manual_cmd += ['--convert-thumbnails', 'jpg']
        manual_cmd += ['--load-info-json', tmp_path]
        url = _manual_entry_url(meta, vid)
        manual_cmd.append(url)
        print(f"[yt_download] Manual chapters: splitting {vid}")
        rc, log_lines = _execute(manual_cmd)
        if rc != 0:
            needs_retry = any('Requested format is not available' in ln for ln in log_lines)
            if needs_retry:
                fallback_cmd = _strip_format(manual_cmd)
                print(f"[yt_download] Manual chapters: retrying {vid} without explicit format selection…")
                rc, log_lines = _execute(fallback_cmd)
            if rc != 0:
                print(f"[yt_download] Manual chapters: yt-dlp exited {rc} for {vid}")
                overall_rc = rc
        try:
            os.remove(tmp_path)
        except Exception:
...
def run(argv: List[str]) -> int:
    ...
    manual_entries = _load_manual_entries(ns.manual_chapters) if getattr(ns, 'manual_chapters', None) else {}
    if getattr(ns, 'manual_chapters', None) and not manual_entries:
        print(f"[yt_download] Warning: manual chapters file '{ns.manual_chapters}' contained no usable entries.")
    manual_id_by_url: Dict[str, str] = {}
    for vid, meta in list(manual_entries.items()):
        info = meta.get('info')
        if isinstance(info, dict):
            manual_entries[vid]['info'] = _normalize_manual_chapters(info)
        url = str(meta.get('url') or '')
        if url:
            manual_id_by_url[url] = vid
    ...
    if has_split and (has_embed_meta or has_embed_thumb):
        ...
        if not _has_flag(cmd, '--write-info-json'):
            cmd += ['--write-info-json']
    # URLs (skip ones we will handle via manual chapters when possible)
    filtered_urls: List[str] = []
    deferred_manual_ids: set[str] = set()
    for u in urls:
        vid = _extract_video_id(u)
        matched_vid: Optional[str] = None
        if vid and vid in manual_entries:
            matched_vid = vid
        elif u in manual_id_by_url:
            matched_vid = manual_id_by_url[u]
        if matched_vid:
            deferred_manual_ids.add(matched_vid)
            continue
        filtered_urls.append(u)
    if manual_entries and deferred_manual_ids:
        print(f"[yt_download] Manual chapters: deferring {len(deferred_manual_ids)} video(s) to custom splitting.")
    unmatched_manual = set(manual_entries.keys()) - deferred_manual_ids
    if manual_entries and unmatched_manual and filtered_urls:
        print(f"[yt_download] Manual chapters: {len(unmatched_manual)} video(s) remain in primary download (likely playlist URLs); will reprocess afterwards.")
    base_cmd = cmd.copy()
    if filtered_urls:
        cmd = base_cmd + filtered_urls
    else:
        cmd = base_cmd
    ...
    manual_base_cmd = cmd[:-len(filtered_urls)] if filtered_urls else cmd[:]
    ...
    if filtered_urls:
        ...
        print('[yt_download] Skipping primary yt-dlp run; manual chapters only.')

    if rc == 0 and manual_entries:
        target_root = stage_dir if stage_dir else Path(dest)
        manual_rc = _run_manual_chapter_jobs(manual_base_cmd, manual_entries, target_root)
        if manual_rc != 0:
            rc = manual_rc
